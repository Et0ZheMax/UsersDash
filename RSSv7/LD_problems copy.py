...
2. «Кластеры» ошибок за окно времени (cluster_regex_list).
3. ≥4 Game Crash за 10 мин.
4. Health-check.
5. 🆕 Корректная подстановка имён аккаунтов вместо raw-ID.
"""
...
def norm_id(raw: str | None) -> str:
    """Приводим ID к нижнему регистру, оставляем только 0-9 a-f."""
    return re.sub(r'[^0-9a-f]', '', (raw or '').lower())

def extract_mapping_recursive(obj, mapping: dict[str, str]) -> None:
    """
    Рекурсивно обходит объект, собирая пары (norm_id(id) → name),
    где id-поле называется 'id', 'accountId', 'account_id' и т.п.
    """
    if isinstance(obj, dict):
        ...
        # рекурсивно проходим дочерние элементы
        for v in obj.values():
...
def load_account_mapping() -> dict[str, str]:
    """Читает PROFILE_FILE и возвращает словарь norm_id → name."""
...
async def check_logs_and_notify() -> None:
    ...
    cluster_dict: defaultdict[str, list[datetime]] = defaultdict(list)
                ...
                for line in f:
                    ...
                    if "Launch: We detected a Game Crash" in line:
                        ...
                        if m:
                            ...
                            acct_norm = norm_id(acct_id_cr)
                            acct_name = id_map.get(acct_norm, acct_norm or "Unknown")
                            try:
    ...
    # ───────── Кластеры ошибок ─────────
    if cluster_dict:
        for acct, tl in cluster_dict.items():
            tl.sort()
            for i in range(len(tl) - (CLUSTER_MIN_COUNT - 1)):
                if tl[i + CLUSTER_MIN_COUNT - 1] - tl[i] <= timedelta(minutes=CLUSTER_WINDOW_MIN):
                    found.append({
                        "file": "CLUSTER",
                        "account": acct,
                        "line": f"{tl[i].isoformat()} | MULTI>{CLUSTER_MIN_COUNT} за {CLUSTER_WINDOW_MIN}м",
                    })
                    break
    else:
        print("⏭️  Кластерная проверка пропущена (нет событий или пустой список шаблонов)")
