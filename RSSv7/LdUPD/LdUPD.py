...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("LDPlayer Batch Installer")
        self.geometry("750x550")
        self.config_dir = tk.StringVar(value=settings.get('config_dir', ''))
        self.xapk_file = tk.StringVar(value=settings.get('xapk_file', ''))
    ...
    def _build_ui(self):
        ...
        tk.Label(frm, text="XAPK file:").grid(row=1, column=0, sticky='e')
        tk.Entry(frm, textvariable=self.xapk_file, width=60).grid(row=1, column=1, padx=5)
        tk.Button(frm, text="Browse…", command=self.browse_xapk).grid(row=1, column=2)

        tk.Label(frm, text="ldconsole:").grid(row=2, column=0, sticky='e')
        tk.Entry(frm, textvariable=self.ldconsole, width=60).grid(row=2, column=1, padx=5)
        tk.Button(frm, text="Browse…", command=self.browse_ldconsole).grid(row=2, column=2)

        tk.Button(frm, text="Scan emulators", command=self.scan_emus).grid(row=3, column=1, pady=5)

        # Панель с чекбоксами и скроллбар слева
    ...
    def browse_xapk(self):
        f = filedialog.askopenfilename(title="Select .xapk file", filetypes=[("XAPK","*.xapk")])
        if f:
            self.xapk_file.set(f)
            settings['xapk_file'] = f
            save_settings()
    ...
    def start_install(self):
        xapk = self.xapk_file.get()
        ld = self.ldconsole.get()
        if not os.path.isfile(xapk) or not os.path.isfile(ld):
            messagebox.showerror("Error", "XAPK or ldconsole.exe not found")
            return
        ...
        threading.Thread(target=self._do_install, args=(selected, ld, xapk), daemon=True).start()

    def _do_install(self, targets, ldpath, xapk):
        for name in targets:
            self._log(f"\n>>> Installing on: {name}")
            cmd = [ldpath, 'installapp', '--name', name, '--filename', xapk]
    ...
    def _mark_installed(self, name):
        # Покрасить в зеленый соответствующий чекбокс
