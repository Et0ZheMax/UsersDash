...
import logging
logger = logging.getLogger("RssV7")
...
PROFILES_DIR = os.path.join(SETTINGS_DIR, "profiles")     # профили аккаунтов *.json (твои же, от GnBots)
...
############################################
# --- CONFIG (template updater) ---
############################################

GNBOTS_PROFILES_PATH = r"C:\Program Files (x86)\GnBots\config\FRESH_NOX.json"  # поправь, если путь иной
SCHEMA_CACHE_PATH    = r"C:\Users\Administrator\Desktop\RssCounterV8\schema_cache.json"  # где держать слепок
...
def schema_learn_from_steps(steps: list):
    """steps — это список шагов из аккаунта (как в /api/manage/account/<id>/settings 'Data').
    ...
    for step in steps:
        ...
        for key, spec in schema[sid]["fields"].items():
            if key not in cfg:
                ...
                # если это select, а options отсутствуют — добавим options из схемы
...
def merge_template_into_account(account_steps: list, template_steps: list):
    """Мёрдж шаблона в аккаунт БЕЗ удаления неизвестных полей/скриптов.
       Сопоставление по ScriptId; при дублях — по порядку появления.
    """
    ...
    for t in tpl:
        ...
        if sid in acc_map and acc_map[sid]:
            ...
            # перезаписываем/добавляем только ключи из шаблона; остальные ключи a_cfg — сохраняем
            for k, v in t_cfg.items():
                a_cfg[k] = v
            # расписания/доп.поля — оставим как есть, чтобы не ломать
            result.append(a)
        else:
            # в аккаунте нет такого ScriptId — добавим шаг целиком
            result.append(t)

    # добавим остальные шаги аккаунта (не упомянутые в шаблоне)
    for s in acc:
...
def _build_schema():
    """
    Собираем «живую схему»:
    - читаем файл профилей (GNBOTS_PROFILES_PATH)
    - для каждого аккаунта парсим Data (строка/массив/вложенность)
    - сводим поля Config по ScriptId (множество ключей)
    """
    import os, time, json

    src = _safe_json_load(GNBOTS_PROFILES_PATH) or {}
    scripts = {}  # ScriptId -> set(field_names)

    # 1) Явная структура "Accounts"/"Profiles"
    candidates = []
    ...
    # 2) Если явного списка нет — пройдёмся рекурсивно по всему файлу
    if not candidates:
        candidates = [src]

    # 3) Собираем шаги со всех кандидатов
    all_steps = []
    ...
    # 5) В JSON множества не лезут — конвертируем в списки
    json_scripts = {sid: sorted(list(fields)) for sid, fields in scripts.items()}

    # 6) Кладём кэш — и обязательно создаём папку!
    try:
...
def _extract_steps_from_obj(obj, acc_id_hint=None):
    """
    Возвращает список шагов (dict с ключами ScriptId/Config) из произвольной структуры.
    Умеет: dict, list, и JSON внутри str (особенно поле Data, где лежит строка с массивом шагов).
    """
    steps = []
    ...
    def try_parse_json_string(s: str):
        ...
        if not s_strip:
            return None
        # Эвристика: строка выглядит как JSON и есть упоминание ScriptId
        ...
        try:
            ...
            # Иногда внутри двойное экранирование — пробуем «разэкрашить»
            try:
    ...
    def walk(node, parent_key=None):
        ...
        if isinstance(node, dict):
            # Если это уже шаг — сохраняем
            if is_step(node):
                steps.append(node)
                # Не return — внутри Config тоже могут быть под-шаги
            # Спец-кейс: поле Data — часто строка с JSON массивом
            ...
            # Вдруг наткнулись на JSON прямо строкой без ключа Data
            parsed = try_parse_json_string(node)
            if parsed is not None:
                walk(parsed, parent_key=parent_key)
        # Остальное игнорируем (int/float/None/bool)
...
def _build_live_schema() -> dict:
    """
    Возвращает effective-схему вида:
    {
      "Scripts": {
         "<ScriptId>": {
             "Defaults": { ... },     # дефолтные значения для Config
             "Shape":    { ... },     # типизация: bool/number/string/select{options}
         },
         ...
      }
    }
    ...
    for step in steps:
        ...
        for k, v in cfg.items():
            ...
            if isinstance(v, dict) and "options" in v:
                ...
                # дефолт — первая опция или текущее value
...
def _need_refresh(now:int) -> bool:
    # по TTL или по изменению mtime источника
...
def _ensure_schema(now:int=None, force:bool=False) -> dict:
    """
    Возвращает актуальную схему. Пересобирает по TTL/mtime или по force.
    """
    global _schema_cache
    now = now or int(time.time())

    if _schema_cache["schema"] is None:
        # Пытаемся поднять кеш с диска (переживает рестарт сервиса)
...
def _normalize_template_with_schema(template_steps:list, schema:dict) -> list:
    """
    На вход — список шагов (как в твоих PREM/650/…),
    Возвращаем список шагов, дополненный актуальными ключами/скриптами из schema.
    """
    scripts_schema = schema.get("Scripts", {})
    # Словарь текущего шаблона по ScriptId (берём «первый» шаг каждого вида — остальное не трогаем)
    by_sid = {}
    ...
    # 1) Для каждого ScriptId в схеме — убедимся, что он представлен в шаблоне (хотя бы 1 шаг)
    for sid, sdef in scripts_schema.items():
        if sid not in by_sid:
            # Добавляем "пустой" шаг с дефолтной конфигурацией
    ...
    # 2) Для каждого шага — дополняем/валидируем Config относительно схемы
    for step in out:
        sid = step.get("ScriptId")
            ...
            if k not in cfg:
                ...
                if form.get("type") == "select":
                    ...
                    if not isinstance(v, dict) or "value" not in v:
                        ...
                        opts = set(form.get("options", []))
                        val  = v.get("value")
                        if val not in opts:
                            # если в шаблоне значение не из options — заменяем на дефолт
                            cfg[k]["value"] = defaults.get(k, next(iter(opts), val))
                        # options — обновляем до актуальных
                        cfg[k]["options"] = list(form.get("options", []))
                elif form.get("type") == "bool":
                    cfg[k] = bool(cfg.get(k, defaults.get(k, False)))
                elif form.get("type") == "number":
                    v = cfg.get(k, defaults.get(k, 0))
                    try:
                        cfg[k] = int(v) if isinstance(v, (int, float, str)) else defaults.get(k, 0)
                    except Exception:
                        cfg[k] = defaults.get(k, 0)
                else:
                    # string
...
# Дата-время в начале строки лога без таймзоны (для кэша и снапшотов):
_DT_RE = re.compile(r'^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{3}) ')
...
def remove_readonly(folder_path):
    """
    Рекурсивно снимает ReadOnly/атрибуты только-чтение со всех файлов/папок.
    Полезно перед копированием/перезаписью конфигов.
    """
...
# Применить шаблон к аккаунту (Id — СТРОКА!)
@app.route("/api/manage/account/<acc_id>/apply_template", methods=["POST"])
def api_apply_template(acc_id):
    """
    Тело запроса (JSON):
    {
      "template": "TRAIN",           # имя шаблона (если не переданы steps)
      "steps": [...],                # можно передать готовые шаги
      "force_schema_refresh": false, # форс-сборка «живой» схемы
      "dry_run": false               # вернуть нормализованные шаги, не сохраняя
    }
    """
    acc_id = str(acc_id)
    # 0) Разбор JSON
    ...
    provided_steps = payload.get("steps")
    force_refresh  = bool(payload.get("force_schema_refresh", False))
    dry_run        = bool(payload.get("dry_run", False))

    # 1) Получаем исходные шаги шаблона
    try:
        if isinstance(provided_steps, list):
            template_steps = provided_steps
        elif isinstance(template_name, str) and template_name.strip():
            template_steps = load_template_by_name(template_name.strip())
        else:
            return jsonify({"ok": False, "error": "Either 'template' (name) or 'steps' array must be provided"}), 400
    except Exception as e:
        return jsonify({"ok": False, "error": f"Failed to load template '{template_name}': {e}"}), 400

    if not isinstance(template_steps, list) or not template_steps:
        return jsonify({"ok": False, "error": "Template steps are empty or invalid"}), 400

    # 2) «Живая» схема (по TTL/mtime или форс)
    try:
        schema = _ensure_schema(force=force_refresh)
    except Exception as e:
        app.logger.exception("schema build failed")
        return jsonify({"ok": False, "error": f"Schema build failed: {e}"}), 500

    # 3) Нормализация шаблона по схеме
    try:
        normalized_steps = _normalize_template_with_schema(template_steps, schema)
    except Exception as e:
        app.logger.exception("template normalization failed")
        return jsonify({"ok": False, "error": f"Template normalization failed: {e}"}), 500

    # 4) Dry-run — только превью
    if dry_run:
        return jsonify({
            "ok": True,
            "dry_run": True,
    ...
    # 5) Сохраняем в профиль (поле Data — строка JSON!)
    try:
        with open(PROFILE_PATH, "r", encoding="utf-8") as f:
            profile = json.load(f)

        acc = next((a for a in profile if a.get("Id") == acc_id), None)
        if not acc:
            return jsonify({"ok": False, "error": f"Account {acc_id} not found"}), 404

        acc["Data"] = json.dumps(
            normalized_steps,
            ensure_ascii=False,
            separators=(",", ":")  # компактно, как в твоих рабочих файлах
        )
        ...
        app.logger.exception("saving account failed")
        return jsonify({"ok": False, "error": f"Saving to account {acc_id} failed: {e}"}), 500

    # 6) Небольшой summary (без падений)
    summary = {}
    try:
        scripts = {}
        for step in normalized_steps:
            ...
            if not sid:
                continue
            cfg = (step.get("Config") or {}) if isinstance(step.get("Config"), dict) else {}
            entry = scripts.setdefault(sid, {"fields": set()})
            for k in cfg.keys():
                entry["fields"].add(k)
        summary = {
            "scripts_total": len(scripts),
            "fields_total": sum(len(v["fields"]) for v in scripts.values())
        }
    except Exception:
        summary = {}
...
def api_schema_rebuild():
    ...
    Полезно, если ты накидал туда свежий «полный» профиль из GUI.
    """
    schema = {}
    changed = False
...
@app.route("/api/manage/account/<acc_id>/settings", methods=["GET"])
def api_get_account_settings(acc_id):
    """Отдаём настройки аккаунта и ПАРАЛЛЕЛЬНО обучаем схему по живым данным,
    чтобы она росла автоматически, без ручного «включить всё в GUI»."""
    data = load_account_settings(acc_id)  # ожидается { "Data": [...] }
    try:
        steps = (data or {}).get("Data") or []
        schema_learn_from_steps(steps)
    except Exception as e:
        print("[SCHEMA] learn failed for acc", acc_id, "->", e)
    return jsonify(data)

@app.route("/api/manage/account/<acc_id>/apply_template", methods=["POST"])
def api_apply_template(acc_id):
    """Применяем шаблон к аккаунту: безопасный «инфляционный» мёрдж.
    body: { "template": "TRAIN.json" } ИЛИ { "template_inline": {...шаги...} }
    """
    try:
        payload = request.get_json(silent=True) or {}
        tpl_name = (payload.get("template") or "").strip()
        tpl_inline = payload.get("template_inline")

        # 1) грузим актуальные шаги аккаунта
        acc = load_account_settings(acc_id) or {}
        acc_steps = acc.get("Data") or []

        # 2) схема (наученная)
        schema = schema_load()

        # 3) сам шаблон — из файла или inline
        if tpl_inline:
            template_steps = tpl_inline
        else:
            if not tpl_name:
                return jsonify({"error": "template or template_inline required"}), 400
            tpl_path = os.path.join(TEMPLATES_DIR, tpl_name)
            if not os.path.isfile(tpl_path):
                return jsonify({"error": "template not found"}), 404
            template_steps = _json_read_or(tpl_path, [])
            if not isinstance(template_steps, list):
                return jsonify({"error": "template invalid"}), 400

        # 4) инфляция шаблона по схеме (добавим отсутствующие ключи)
        template_filled = template_inflate_with_schema(template_steps, schema)

        # 5) безопасный мёрдж в аккаунт
        merged_steps = merge_template_into_account(acc_steps, template_filled)

        # 6) сохранить профиль (сделаем бэкап файла, если читаешь из файла)
        try:
            backup_profile_file_if_needed(acc_id)  # если у тебя есть файловое хранилище, реализуй ниже; иначе NOP
        except Exception as e:
            print("[APPLY] backup failed for acc", acc_id, "->", e)

        acc["Data"] = merged_steps
        save_account_settings(acc_id, acc)  # твоя функция сохранения (в БД/файл)
